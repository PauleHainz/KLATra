...
# Regionalwind ist noch nicht implementiert!
if 0 not in iozeit:
      ...
      if len(x1) == 0:
        ...
        for p_traj in range(0,len(x1)):
          ...
          if Startzeit[p_traj] < Endzeit[p_traj]:
            ...
            if mgv == 1:
              trajektorie.fillna(-55.5, inplace=True)
        ...
        if "KLAM" in out_format:
          for i in range(0, len(traj_list)):
            test = pd.DataFrame(traj_list[i].loc[traj_list[i].index==0])
            if Startzeit[i] < Endzeit[i]:
              for dti in range(dt+int(test.loc[0,"Zeit"]),int(test.loc[0,"Zeit"])+int(max(traj_list[i].dt)),dt):
                test = pd.concat([test, traj_list[i].loc[np.argsort(np.argsort(round(abs(traj_list[i]['Zeit']-dti))))==0]])
            else:
              for dti in range(int(test.loc[0,"Zeit"])-dt,int(test.loc[0,"Zeit"])-int(max(traj_list[i].dt)),-dt):
                test = pd.concat([test, traj_list[i].loc[np.argsort(np.argsort(round(abs(traj_list[i]['Zeit']-dti))))==0]])
            test = pd.concat([test,  traj_list[i].loc[traj_list[i].index==max(traj_list[i].index)]], ignore_index=True)
            ...
            if "Ueberwaermungsgebiete" in in_nml["input"]:
              for q in range(len(test)):
                ...
                if len(bezirk.name.values) > 0:        
                    test.loc[q,"Überwärmungsgebiet"] = bezirk.name.values[0]
                else:
                    test.loc[q,"Überwärmungsgebiet"] = "NaN"
            else: 
              test["Überwärmungsgebiet"] = "nb"
            if "Quellgebiete" in in_nml["input"]:
              ...
              test["Quellgebiet"] = gebiet.QG_kurz.values[0]
            else:
              test["Quellgebiet"] = "nb"
            if in_nml["output"]["Farbverlauf_mit_Zeit"] == True:
              ...
              if Startzeit[i] < Endzeit[i]:
                ...
                for n in range(0, len(zeiten.zeit)):
                  ...
                  test1.fillna(-99.9,inplace=True)
                  if Startzeit[i] < Endzeit[i]:
        ...
        if "geojson" in out_format:
          ...
          points_df.fillna(-99.9, inplace=True)
